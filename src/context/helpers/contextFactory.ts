...
export const contextFactory = <CtxState, DispatchReducer>(initialState) => {
  const context = createContext<{state: CtxState; dispatch: DispatchReducer}>({
    state: initialState,
    dispatch: () => null,
  });
...
//   const useCtxSelector = <Selected>(contextSelector: ContextSelector<Selected, CtxState>) => {
...
//     // return useContextSelector<CtxState | undefined, ReturnType<typeof selector>>(context, selector)
//   }
